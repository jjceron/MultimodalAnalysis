...
BEST_CONFIG = {
    "name": "eegnet_small_t600_meanmax050",
    "F1": 8,
    "D": 2,
    "F2": 16,
    "temporal_kernel": 63,
...
def run_one_training(
    ...
    print(f"  temporal_kernel:  {config['temporal_kernel']}")
    print(f"  separable_kernel: {config['separable_kernel']}")
    print(f"  pool1:            {config['pool1']}")
    print(f"  pool2:            {config['pool2']}")
    print(f"  total_pool:       {config['pool1'] * config['pool2']}")
    print(f"  expected T':      approx 38400 / {config['pool1'] * config['pool2']}")
    print(f"  aggregation:      meanmax")
    print(f"  meanmax_alpha:    {meanmax_alpha}")
...
def summarize_results(
    ...
    grouped_overall = pd.DataFrame(
        [
            {
                "config": summary_df["config"].iloc[0],
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=[3407, 1234, 2025, 777, 42],
        help="Semillas que cambian las particiones StratifiedGroupKFold.",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[3407],
        help="Semillas que cambian inicialización, dropout y orden de batches.",
    )
    ...
    parser.add_argument("--lr", type=float, default=BEST_CONFIG["lr"])
    parser.add_argument("--weight-decay", type=float, default=BEST_CONFIG["weight_decay"])
    parser.add_argument("--dropout", type=float, default=BEST_CONFIG["dropout"])
    parser.add_argument("--meanmax-alpha", type=float, default=BEST_CONFIG["meanmax_alpha"])
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="outputs/eeg_ct/eegnet_small_t600_meanmax050_confirmatory",
    )
    ...
    print("\nFixed model under verification")
    print(f"  name:             {BEST_CONFIG['name']}")
    print(f"  F1:               {BEST_CONFIG['F1']}")
    print(f"  D:                {BEST_CONFIG['D']}")
    print(f"  F2:               {BEST_CONFIG['F2']}")
    print(f"  temporal_kernel:  {BEST_CONFIG['temporal_kernel']}")
    print(f"  separable_kernel: {BEST_CONFIG['separable_kernel']}")
    print(f"  pool1:            {BEST_CONFIG['pool1']}")
    print(f"  pool2:            {BEST_CONFIG['pool2']}")
    print(f"  total_pool:       {BEST_CONFIG['pool1'] * BEST_CONFIG['pool2']}")
    print(f"  aggregation:      meanmax")
